...
var serverEndpoint = '/api';
var manifest = fetch(`${serverEndpoint}/manifests/aliens-manifest.json`)
  .then((response) => {
    if (!response.ok) {
...
const experiment = async () => {  
  ...
  function saveData(filename, data){
    ...
    xhr.open('POST', `${serverEndpoint}/upload`);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.send(JSON.stringify({filedata: data, filename: filename, taskname: 'two-step'}));
  };
  ...
  var month = String(date.getMonth() + 1).padStart(2, '0'); // Add 1 to the month, since getMonth returns 0-11
  var year = date.getFullYear();
  var datestring = [year, month, day, hour, minute].join("");
  
  console.log(datestring); // Output will be in the format yyyymmddHHMM
  ...
  for (let i=0; i < outcomes.length; i++){
    ...
    const trial = {
      ...
      on_finish: function(data) {
        ...
        saveData('aliens-manifest.json', JSON.stringify(manifested));
      }
    }
